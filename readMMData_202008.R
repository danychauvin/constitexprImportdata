...
#Proper column names
properColNames <- c("file_path","lane_ID","cell_ID","frame","cell_rank","genealogy","type_of_end","parent_ID","cells_in_lane","bbox_top px","bbox_bottom px","center_x px","center_y px","width px","length px","tilt rad","area px^2","bgmask_area px^2","fluo_cellmask_1","fluo_bgmask_ch_1","fluo_ampl_ch_1","fluo_bg_ch_1","cell_rank")
...
myframes <- myconditions %>% 
  ...
  #lapply(function(.l) readr::read_delim(.l,skip=2,delim=";",show_col_types =FALSE) %>% 
           #mutate(file_path=.l)) %>% 
  lapply(function(.l) as_tibble(data.table::fread(.l,skip=2,sep=";")) %>% 
  mutate(file_path=.l) %>% 
