...
#Proper column names
properColNames <- c("lane_ID","cell_ID","frame","cell_rank","genealogy","type_of_end","parent_ID","cells_in_lane","bbox_top_px","bbox_bottom_px",
                    "touches_detection_roi_top","center_x_px","center_y_px","width_px","length_px","tilt_rad","oriented_bbox_center_x_px","oriented_bbox_center_y_px",
                    "oriented_bbox_width_px","oriented_bbox_length_px","oriented_bbox_orientation_angle_rad","area_px","bgmask_area_px",
                    "phc_total_intensity_au","phc_intensity_coefficient_of_variation","label:dead","label:dying","label:fading","label:shrinking","label:dividing",
                    "fluo_cellmask_ch_1","fluo_bgmask_ch_1","fluo_ampl_ch_1","fluo_bg_ch_1","fluo_cellmask_ch_2","fluo_bgmask_ch_2","fluo_ampl_ch_2","fluo_bg_ch_2")
...
myframes <- myconditions %>% 
  ...
  #lapply(function(.l) readr::read_delim(.l,skip=2,delim=";",show_col_types =FALSE) %>% 
           #mutate(file_path=.l)) %>% 
  lapply(function(.l) as_tibble(data.table::fread(.l,skip=4,sep=",")) %>% 
  mutate(file_path=.l) %>% 
...
myframes <- myframes %>% 
  ...
  mutate(length_um=length_pixel*dl,
         ...
         volume_um=compute_vol(length_um,width_um)) %>%
...
myframes <- myframes %>% 
  ...
  mutate(start_time=first(time_sec), end_time=last(time_sec),
         ...
         volume_predict=fit_exp_elongation_predict(time_sec,volume_um),
         alpha=fit_exp_elongation_slope(time_sec,volume_um), 
         sd_alpha=fit_exp_elongation_sd_slope(time_sec,volume_um)) %>%  #using Michael ellipse fitting
  collect() %>% 
...
mycells <- myframes_to_mycells %>% 
  ...
  do((function(.df) {
    ...
    .alpha=first(.df$alpha)
    .sd_alpha=first(.df$sd_alpha)
    data.frame(npoints=.mod_ll_t$df.residual+1,
               ...
               w_birth=first(.df$width_um),w_div=last(.df$width_um),
               volume_birth=first(.df$volume_predict), volume_div=last(.df$volume_predict),
...
mycells <- mycells %>% 
  ...
  mutate(dl=l_div - l_birth,
         dv=volume_div - volume_birth,
         alpha_len=log(l_div/l_birth) / div_time,
         alpha_vol=log(volume_div/volume_birth)/div_time) 
